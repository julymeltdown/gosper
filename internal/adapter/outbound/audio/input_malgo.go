...
func (MalgoInput) Open(ctx context.Context, deviceID string, fmt domain.AudioFormat) (port.AudioStream, error) {
    contextConfig := malgo.ContextConfig{}
    malgoCtx, err := malgo.InitContext(nil, contextConfig, nil)
    if err != nil { return nil, err }
    ...
    if fmt.Channels > 0 { cfg.Capture.Channels = uint32(fmt.Channels) }
    if fmt.SampleRate > 0 { cfg.SampleRate = uint32(fmt.SampleRate) } else { cfg.SampleRate = 16000 }
    ...
    if deviceID != "" {
        ...
        if _, err := fmt.Sscanf(deviceID, "index:%d", &idx); err == nil {
            ...
            if err == nil {
                ...
                for i, d := range devs { list = append(list, domain.Device{ID: fmt.Sprintf("index:%d", i), Name: d.Name()}) }
        ...
        Data: func(pOutput, pInput []byte, frameCount uint32) {
            ...
            if cfg.Capture.Channels > 1 {
                ...
                for f := 0; f < len(mono); f++ {
                    ...
                    for c := 0; c < int(cfg.Capture.Channels); c++ { sum += out[base+c] }
                    mono[f] = sum / float32(cfg.Capture.Channels)
                }
                select { case framesCh <- mono: default: }
            } else {
                select { case framesCh <- out: default: }
            }
        },
    }

    dev, err = malgo.InitDevice(malgoCtx.Context, cfg, callbacks)
    if err != nil { malgoCtx.Uninit(); return nil, err }
    if err := dev.Start(); err != nil { dev.Uninit(); malgoCtx.Uninit(); return nil, err }
...
func (s *malgoStream) Close() error {
    ...
    if s.ch != nil { close(s.ch); s.ch = nil }
    return nil
}
